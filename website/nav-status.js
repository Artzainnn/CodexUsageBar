...
// in sync with status.openai.com. Pings every 60s; per-tab localStorage
...
(function() {
  ...
  function getSelectedIds(components) {
    ...
    return new Set(components.filter(c => /codex|vs code|login|conversations/i.test(c.name)).map(c => c.id));
  }
