...
# Build script for CodexUsageBar

echo "Building CodexUsageBar..."
...
if [ -f "CodexUsageBar.icns" ]; then
    cp CodexUsageBar.icns "$APP_PATH/Contents/Resources/"
    # Update Info.plist to reference icon
    /usr/libexec/PlistBuddy -c "Add :CFBundleIconFile string CodexUsageBar" "$APP_PATH/Contents/Info.plist" 2>/dev/null || \
    /usr/libexec/PlistBuddy -c "Set :CFBundleIconFile CodexUsageBar" "$APP_PATH/Contents/Info.plist"
fi

# Compile the Swift app for arm64
swiftc -parse-as-library -o "$APP_PATH/Contents/MacOS/CodexUsageBar_arm64" \
    CodexUsageBar.swift \
    -framework SwiftUI \
    -framework AppKit \
    -framework WebKit \
...
swiftc -parse-as-library -o "$APP_PATH/Contents/MacOS/CodexUsageBar_x86_64" \
    CodexUsageBar.swift \
    -framework SwiftUI \
    -framework AppKit \
    -framework WebKit \
...
lipo -create -output "$APP_PATH/Contents/MacOS/CodexUsageBar" \
    "$APP_PATH/Contents/MacOS/CodexUsageBar_arm64" \
    "$APP_PATH/Contents/MacOS/CodexUsageBar_x86_64"

# Clean up individual arch binaries
rm "$APP_PATH/Contents/MacOS/CodexUsageBar_arm64"
rm "$APP_PATH/Contents/MacOS/CodexUsageBar_x86_64"

# Create PkgInfo file
echo -n "APPL????" > "$APP_PATH/Contents/PkgInfo"

# Set proper permissions first
chmod 755 "$APP_PATH/Contents/MacOS/CodexUsageBar"
